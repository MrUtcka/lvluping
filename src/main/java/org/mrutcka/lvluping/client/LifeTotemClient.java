...
public final class LifeTotemClient {

    private static final Map<UUID, TotemVisual> ACTIVE = new ConcurrentHashMap<>();

    public static void show(UUID id, double x, double y, double z, float yRot, long untilGameTime) {
        ACTIVE.put(id, new TotemVisual(x, y, z, yRot, untilGameTime));
    }
    ...
    public static void tick() {
        ...
        Iterator<Map.Entry<UUID, TotemVisual>> it = ACTIVE.entrySet().iterator();
        while (it.hasNext()) {
    ...
    public static void onRenderLevelStage(RenderLevelStageEvent event) {
        ...
        ItemStack stack = new ItemStack(LvlupingItems.RANGER_TOTEM_MODEL.get());
        ...
        for (TotemVisual v : ACTIVE.values()) {
            if (v.untilGameTime <= time) continue;

            pose.pushPose();
            pose.translate(v.x - camX, v.y - camY + 0.02, v.z - camZ);
            pose.mulPose(Axis.YP.rotationDegrees(-v.yRot + 180f));
            pose.mulPose(Axis.XP.rotationDegrees(270f));
    ...
    private record TotemVisual(double x, double y, double z, float yRot, long untilGameTime) {}
}
