...
public final class MergeTreeClient {

    private static final Map<UUID, TreeVisual> ACTIVE = new ConcurrentHashMap<>();

    public static void show(UUID id, double x, double y, double z, float yRot, long untilGameTime) {
        ACTIVE.put(id, new TreeVisual(x, y, z, yRot, untilGameTime));
    }
    ...
    public static void tick() {
        ...
        Iterator<Map.Entry<UUID, TreeVisual>> it = ACTIVE.entrySet().iterator();
        while (it.hasNext()) {
    ...
    public static void onRenderLevelStage(RenderLevelStageEvent event) {
        ...
        ItemStack stack = new ItemStack(LvlupingItems.RANGER_TREE_MODEL.get());
        ...
        for (TreeVisual v : ACTIVE.values()) {
            if (v.untilGameTime <= time) continue;

            pose.pushPose();
            pose.translate(v.x - camX, v.y - camY + 0.1, v.z - camZ);
            pose.mulPose(Axis.YP.rotationDegrees(-v.yRot + 180f));
            pose.mulPose(Axis.XP.rotationDegrees(90f));
            pose.scale(1.25f, 2.50f, 1.25f);
    ...
    private record TreeVisual(double x, double y, double z, float yRot, long untilGameTime) {}
}
