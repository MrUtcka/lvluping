...
public final class AssassinTripwireClient {
    private static final Map<UUID, Visual> ACTIVE = new ConcurrentHashMap<>();

    public static void show(UUID id, double x, double y, double z, float yRot, long untilGameTime) {
        ACTIVE.put(id, new Visual(x, y, z, yRot, untilGameTime));
    }
    ...
    public static void tick() {
        ...
        Iterator<Map.Entry<UUID, Visual>> it = ACTIVE.entrySet().iterator();
        while (it.hasNext()) {
            if (it.next().getValue().untilGameTime <= t) it.remove();
        }
    }
    ...
    public static void onRenderLevelStage(RenderLevelStageEvent event) {
        ...
        ItemStack stack = new ItemStack(LvlupingItems.ASSASSIN_TRIPWIRE_MODEL.get());
    ...
    private record Visual(double x, double y, double z, float yRot, long untilGameTime) {}
}
